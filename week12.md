...
멀티스레드 프로그램을 작서할때 주의할 점은 다수의 스레드가 공유 데이터에 동시 접근하는 경우에 대한 처리이다. 이에 대한 해결책이 바로 스레드 동기화(Thread Synchroniztion)이다. 

synchronized로 동기화 블록 지정 <br>
wait() - notify() 메소드로 스레드 실행 제어 <br>

자바의 스레드 동기화를 위한 stnchronized 키워드 

가장 쉬운 스레드 동기화 기법은, 스레드가 공유 데이터를 접근할 때, 하나씩 순차적으로 실행하도록 제어하는 기법이다. 한 스레드가 공유 데이터의 접근을 시작한 순간, 데이터를 잠가(lock) 다른 스레드가 대기 하도록 하면 된다.

synchronized 키워드는 스레드 동기화를 위한 장치로서, 임의의 코드 블록을 동기화가 설정된 임계 영역으로 지정한다. synchronized 키워드로 하나의 메소드를 통째로 임계 영역으로 지정하는 방법과 임의의 코드 블럭을 임계 영역으로 지정하는 방법이다. 어떤 방법을 사용하든지 synchronized 블록은 진입할때 락(lock)이 걸리고 빠져나올때 락이 풀리는(unlock)동작이 자동으로 이루어지도록 컴파일된다.
...
wait() - nodify()를 이용한 스레드 동기화가 필요한 경우 
...
Object의 wait(), notift() 메소드 

wait() - notify()를 이영하면 앞의 producer - consumer 문제의 스레드 동기화를 해결할 수 있다. java.lang.Object 클래스는 스레드 사이에 동기화를 위한 3개의 메소드 wait(), notify(), notifyall()를 제공한다. 모든 객체는 Object 클래스를 상속받기 때문에, 자바는 모든 객체가 동기화 객체가 될 수 있도록 설계하였다.

wait()

다른 스레이드가 이 객체의 notify()를 불러줄 때까지 대기한다.

notify()

이객체에 대기 중인 스레드를 깨워 RUNNABLE 상태로 만든다. 2개 이상의 스레드가 대기 중이라도 오직 한 개의 스레드만 깨워 RUNNABLE 상태로 한다.
